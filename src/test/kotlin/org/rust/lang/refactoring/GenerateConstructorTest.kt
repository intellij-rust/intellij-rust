...
import org.rust.lang.refactoring.generateConstructor.RsStructMemberChooserObject
...
impl S {
    ...
    private fun doTest(@Language("Rust") code: String,
        ...
        checkByText(code.trimIndent(), expected.trimIndent()) {
            mockStructMemberChooser({ _, all ->
                TestCase.assertEquals(all.map { it.text }, chooser.map { it.member })
    ...
    private fun extractSelected(all: List<RsStructMemberChooserObject>, chooser: List<ConstructorArgumentsSelection>): List<RsStructMemberChooserObject> {
        val selected = chooser.filter { it.isSelected }.map { it.member }
        return all.filter { selected.contains(it.text) }
    }
