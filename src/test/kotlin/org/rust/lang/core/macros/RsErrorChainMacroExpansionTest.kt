...
class RsErrorChainMacroExpansionTest : RsMacroExpansionTestBase() {
    ...
    @Language("Rust")
    private val ERROR_CHAIN_PROCESSING_MACRO = """
    ...
    fun `test error_chain_processing 1`() = doTest("""
        $ERROR_CHAIN_PROCESSING_MACRO
        error_chain_processing! {
            ({}, {}, {}, {})
            links {
                Inner(inner::Error, inner::ErrorKind) #[doc = "Link to another `ErrorChain`."];
    ...
    fun `test error_chain_processing 2`() = doTest("""
        $ERROR_CHAIN_PROCESSING_MACRO
        error_chain_processing! {
            (
                {},
    ...
    fun `test error_chain_processing 3`() = doTest("""
        $ERROR_CHAIN_PROCESSING_MACRO
        error_chain_processing! {
            (
                {},
    ...
    """, """
        error_chain_processing! {
            (
                ...
                { Inner(inner::Error, inner::ErrorKind) #[doc = "Link to another `ErrorChain`."]; },
                { Io(::std::io::Error) #[doc = "Link to a `std::error::Error` type."]; },
                { #[doc = "A custom error kind."] Custom }
            )
        }
    """)

    fun `test error_chain_processing 4`() = doTest("""
        $ERROR_CHAIN_PROCESSING_MACRO
        error_chain_processing! {
            (
                {},
    ...
    """, """
        impl_error_chain_processed! {
            types  {}
            links  { Inner(inner::Error, inner::ErrorKind) #[doc = "Link to another `ErrorChain`."]; }
            foreign_links  { Io(::std::io::Error) #[doc = "Link to a `std::error::Error` type."]; }
            errors  { #[doc = "A custom error kind."] Custom }

        }
    """)

    fun `test impl_error_chain_processed`() = doTest("""
    ...
    """, """
        ...
        impl  ::ChainedError for  Error {
            ...
            impl_extract_backtrace!( Error
                                     ErrorKind
                                     [ inner::Error,  #[ doc = "Link to another `ErrorChain`."]]);
        }
        #[allow(dead_code)]
        impl  Error {
        ...
        impl ::std::error::Error for  Error {
            ...
            fn cause(&self) -> Option<&::std::error::Error> {
                match self.1.next_error {
                    ...
                    None => {
                        match self.0 {
                             #[ doc = "Link to a `std::error::Error` type."]
        ...
        #[ doc = "Link to another `ErrorChain`."]
        impl From< inner::ErrorKind> for  ErrorKind {
            fn from(e:  inner::ErrorKind) -> Self {
                 ErrorKind:: Inner(e)
            }
        }
