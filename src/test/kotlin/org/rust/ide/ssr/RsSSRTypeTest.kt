...
class RsSSRTypeTest : RsSSRTestBase() {
    ...
    fun `test Path RsLifetime QUOTE_IDENTIFIER`() = doTest("""
        <warning>struct S1(&'a u32);</warning>
        <warning>struct S2(&'b u32);</warning>
        struct S3(&u32);
    """, """struct '_(&\''_ u32)""")
    ...
    fun `test Path RsWherePred`() = doTest("""
        ...
        struct S2<'b, T> where T: 'a {}
        struct S3<'a> where T: 'a {}
        struct S4<'b, U> where U: 'a {}
        struct S4<'a, T> where U: 'a {}
        struct S4<'a, T> where T: 'b {}
    ...
    fun `test Path RsConstParameter`() = doTest("""
        ...
        struct S5<X, Y: usize>([X; Y]);
        struct S6<X, Y>([X; Y]);
        struct S7<X, const Y: usize>([A; Y]);
        struct S8<X, const Y: usize>([X; A]);
