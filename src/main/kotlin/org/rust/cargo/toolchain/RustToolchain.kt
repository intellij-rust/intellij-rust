...
data class RustToolchain(val location: Path) {
    ...
    fun rustup(cargoProjectDirectory: Path): Rustup? =
        if (isRustupAvailable) Rustup(this, pathToExecutable(RUSTUP), cargoProjectDirectory) else null
    ...
    data class VersionInfo(val rustc: RustcVersion?)

    companion object {
        private const val RUSTC: String = "rustc"
        private const val RUSTFMT: String = "rustfmt"
        private const val CARGO: String = "cargo"
        private const val RUSTUP: String = "rustup"
        private const val XARGO: String = "xargo"

        const val CARGO_TOML: String = "Cargo.toml"
        const val CARGO_LOCK: String = "Cargo.lock"
        const val XARGO_TOML: String = "Xargo.toml"

        fun suggest(): RustToolchain? =
            Suggestions.all().mapNotNull {
...
data class RustcVersion(val semver: SemVer, val host: String, val channel: RustChannel)
...
private object Suggestions {
    ...
    fun all(): Sequence<File> =
        sequenceOf(fromRustup(), fromPath(), forMac(), forUnix(), forWindows()).flatten()

    private fun fromRustup(): Sequence<File> {
        val file = File(FileUtil.expandUserHome("~/.cargo/bin"))
        return if (file.isDirectory) sequenceOf(file) else emptySequence()
    }

    private fun fromPath(): Sequence<File> =
        System.getenv("PATH")
            .orEmpty()
