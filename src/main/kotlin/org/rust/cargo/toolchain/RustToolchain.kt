...
data class RustToolchain(val location: Path) {
    ...
    fun cargoOrXargo(cargoProjectDirectory: Path): Cargo {
        val hasXargoToml = cargoProjectDirectory.resolve(XARGO_TOML)?.let { Files.isRegularFile(it) } == true
        val toolName = if (hasXargoToml && hasExecutable(XARGO)) XARGO else CARGO
        return Cargo(pathToExecutable(toolName), pathToExecutable(RUSTC))
    }
