...
open class CargoProjectsServiceImpl(
    ...
    fun updateAllFeatures(cargoProject: CargoProjectImpl, pkg: CargoWorkspace.Package, enable: Boolean) {
//        runWriteAction { saveAllDocuments() }
//        refreshAllProjects()

        val pkgRoot = pkg.rootDirectory.toString()
        val pkgFeatures = pkg.featureState.keys
        val userOverriddenFeatures = cargoProject.userOverriddenFeatures.mapValues { (packageRoot, userFeatures) ->
            if (packageRoot == pkgRoot) {
                when {
                    enable -> pkgFeatures // enable all features in the package
                    else -> emptySet()    // disable all features in the package
                }
            } else {
                userFeatures
            }
