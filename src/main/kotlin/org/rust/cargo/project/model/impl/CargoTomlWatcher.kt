...
/**
 * File changes listener, detecting changes inside the `Cargo.toml` files and creation of `*.rs` files acting as
 * automatic crate root.
 */
class CargoTomlWatcher(private val onCargoTomlChange: () -> Unit) : BulkFileListener {
...
private val IMPLICIT_TARGET_FILES: List<String> =
    listOf("/build.rs", "/src/main.rs", "/src/lib.rs")

private val IMPLICIT_TARGET_DIRS: List<String> =
    listOf("/src/bin", "/examples", "/tests", "/benches")
    ...
    when {
        ...
        IMPLICIT_TARGET_FILES.any { event.pathEndsWith(it) } -> true
            ...
            IMPLICIT_TARGET_DIRS.any {
                parent.endsWith(it) || (event.pathEndsWith("main.rs") && grandParent.endsWith(it))
            }
        }
    }
