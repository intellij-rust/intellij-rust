...
abstract class JoinListIntentionBase<TList : RsElement, TElement : RsElement>(
    ...
    override fun invoke(project: Project, editor: Editor, ctx: TList) {
        ...
        nextBreak(elements.last())?.let { document.deleteString(it.textRange.startOffset, it.textRange.endOffset) }
        elements.dropLast(1).asReversed().forEach {
            nextBreak(it)?.let { document.replaceString(it.textRange.startOffset, it.textRange.endOffset, " ") }
        }
        prevBreak(elements.first())?.let { document.deleteString(it.textRange.startOffset, it.textRange.endOffset) }
    }
}
