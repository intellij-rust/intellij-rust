...
open class AddStructFieldsLiteralIntention : RsElementBaseIntentionAction<AddStructFieldsLiteralIntention.Context>() {
    ...
    override fun findApplicableContext(project: Project, editor: Editor, element: PsiElement): Context? {
        val isAppropriate = element.elementType == RsElementTypes.DOTDOT
            && element.context?.context is RsStructLiteral
        if (!isAppropriate) return null
        if (!PsiModificationUtil.canReplace(element)) return null
        return Context(element.context?.parent as RsStructLiteral)
    }
