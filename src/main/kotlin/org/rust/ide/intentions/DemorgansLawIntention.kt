...
class DemorgansLawIntention : RsElementBaseIntentionAction<DemorgansLawIntention.Context>() {
    ...
    override fun getFamilyName(): String = "DeMorgan's Law"
    ...
    data class Context(val binaryExpr: RsBinaryExpr, val binaryOpType: BinaryOperator)
    ...
    override fun invoke(project: Project, editor: Editor, ctx: Context) {
        ...
        while (topBinExpr.parent is RsBinaryExpr
            ...
            topBinExpr = if (topBinExpr.parent is RsBinaryExpr) {
                topBinExpr.parent
            } else {
                topBinExpr.parent.parent.parent
            } as RsBinaryExpr
        ...
        if (isAllSameOpType) {
            ...
            val binaryExprs = topBinExpr.descendantsOfType<RsBinaryExpr>().filter { binaryExpr ->
                !(binaryExpr.operatorType != opType
                    || binaryExpr.parent is RsBinaryExpr
                    && (binaryExpr.parent as RsBinaryExpr).operatorType == opType)
            }
    ...
    private fun isConjunctionExpression(expr: RsExpr, opType: BinaryOperator): Boolean =
        expr is RsBinaryExpr && expr.operatorType == opType
    ...
    private fun convertLeafExpression(condition: RsExpr): String {
        if (condition.isNegation()) {
            ...
            when (condition) {
                ...
                is RsBinaryExpr -> return condition.negateToString()
                else -> return "!${condition.text}"
            }
        }
    }

    private fun convertConjunctionExpression(binaryExpr: RsBinaryExpr, opType: BinaryOperator): String? {
        ...
        binaryExpr.right ?: return null

        val rhs = binaryExpr.right ?: return null
        ...
        val flippedConjunction = if (binaryExpr.operatorType == opType) {
            if (opType == AND) "||" else "&&"
        } else {
            binaryExpr.operator.text
        }
