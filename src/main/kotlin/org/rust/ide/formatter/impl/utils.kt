...
val PAREN_DELIMITED_BLOCKS =
    ts(VALUE_PARAMETER_LIST, PAREN_EXPR, TUPLE_EXPR, TUPLE_TYPE, ARG_LIST, FORMAT_MACRO_ARGS, TRY_MACRO_ARGS, PAT_TUP, TUPLE_FIELDS)
val PAREN_LISTS = orSet(PAREN_DELIMITED_BLOCKS, ts(PAT_ENUM))

val BRACK_DELIMITED_BLOCKS = ts(VEC_TYPE, ARRAY_EXPR, FORMAT_MACRO_ARGS, TRY_MACRO_ARGS, VEC_MACRO_ARGS)
...
val TYPES = ts(VEC_TYPE, REF_LIKE_TYPE, BARE_FN_TYPE, TUPLE_TYPE, BASE_TYPE,
    TYPE_WITH_BOUNDS_TYPE, FOR_IN_TYPE)

val MACRO_ARGS = ts(MACRO_ARG, FORMAT_MACRO_ARGS, TRY_MACRO_ARGS, VEC_MACRO_ARGS)

val FN_DECLS = ts(FUNCTION, BARE_FN_TYPE, LAMBDA_EXPR)
