...
class RsBraceMatcher : PairedBraceMatcherAdapter(RsBaseBraceMatcher(), RsLanguage) {
    ...
    private fun isBrace(
        ...
        // Here is the tricky part. Unlike `{}`, `()` and `[]`, `<>` does not always form a brace pair because of
        // comparison operators. Another complication is that we don't have access to tree here, and can use only
        // the lexical structure. Luckily, chained comparisons are forbidden in Rust.
        //
        // So let's run a standard stack-based brace matching algorithm with a twist that we try to bail out early
        // if we see some token which is unlikely to be seen between a pair of `<>`.
        var count = 0
        try {
            val braceStack = dequeOf<IElementType>(brace)
            var prevIsAnd = false
            while (true) {
                count++
                ...
                if (current in UNPAIRED_TYPE_TOKENS) {
                    // `&` `&` is glued to `&&` only in the parser, need to handle them specially
    ...
    companion object {
        ...
        private fun coBrace(brace: IElementType): IElementType? =
            when (brace) {
                LT -> GT
                GT -> LT
        ...
        private val OPEN_BRACES: TokenSet = tokenSetOf(LT, LPAREN, LBRACE, LBRACK)

        val UNPAIRED_TYPE_TOKENS: TokenSet =
            TokenSet.orSet(
                RS_COMMENTS,
                tokenSetOf(
                    WHITE_SPACE,
...
private class RsBaseBraceMatcher : PairedBraceMatcher {
    ...
    companion object {
        ...
        private val InsertPairBraceBefore: TokenSet =
            TokenSet.orSet(
                RS_COMMENTS,
                TokenSet.create(
