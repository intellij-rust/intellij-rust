...
class ConvertToTyUsingFromTraitFix(expr: PsiElement, val ty: Ty) : LocalQuickFixAndIntentionActionOnPsiElement(expr) {
    ...
    override fun invoke(
        ...
        val assocFunctionCall = RsPsiFactory(project)
            .createAssocFunctionCall(tyToStringWithoutTypeArgs(ty), "from", listOf(startElement))
        startElement.replace(assocFunctionCall)
    }
}
