...
private const val BYTE_ESCAPE_LENGTH: Int = "\\x00".length
private const val UNICODE_ESCAPE_MIN_LENGTH: Int = "\\u{0}".length
private const val UNICODE_ESCAPE_MAX_LENGTH: Int = "\\u{000000}".length
...
class RustEscapesLexer private constructor(
    ...
    override fun determineTokenType(): IElementType? {
        ...
        if (bufferSequence[tokenStart] != '\\') return defaultToken

        // \ is at the end of the string token
        if (tokenStart + 1 >= tokenEnd) return INVALID_CHARACTER_ESCAPE_TOKEN
    ...
    private fun isValidUnicodeEscape(start: Int, end: Int): Boolean {
        val unicodeEscapeLengthRange = UNICODE_ESCAPE_MIN_LENGTH..UNICODE_ESCAPE_MAX_LENGTH
        return bufferSequence.substring(start, end).count { it != '_' } in unicodeEscapeLengthRange
    ...
    private fun testCodepointRange(start: Int, end: Int, max: Int): Boolean =
        try {
            ...
            if (range.startsWith('_')) false else Integer.parseInt(range.filter { it != '_' }, 16) <= max
    ...
    companion object {
        ...
        /**
         * Create an instance of [RustEscapesLexer] suitable for situations when there is no need to care about token
         * types.
         *
        ...
        val ESCAPABLE_LITERALS_TOKEN_SET: TokenSet =
            TokenSet.create(
                BYTE_LITERAL,
                CHAR_LITERAL,
                STRING_LITERAL,
...
private fun Char.isRustWhitespaceChar(): Boolean =
