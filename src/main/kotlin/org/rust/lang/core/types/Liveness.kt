...
class LivenessContext private constructor(
    ...
    fun check(): Liveness? {
        val livenessData = GatherLivenessContext(this).gather()
...
class FlowedLivenessData(
    ...
    private fun isDead(usagePath: UsagePath): Boolean {
        var isDead = true
        return dfcxLivePaths.eachBitAtFlowExit { index ->
...
sealed class UsagePath {
    data class Base(val declaration: RsPatBinding) : UsagePath()
    data class Extend(val parent: UsagePath) : UsagePath()

    private val base: UsagePath.Base
        get() = when (this) {
            is UsagePath.Base -> this
            is UsagePath.Extend -> parent.base
        }
            ...
            return when (category) {
                is Categorization.Rvalue -> {
                    ...
                    return when (declaration) {
                        is RsPatBinding -> UsagePath.Base(declaration)
                        else -> null
                    }
                }
...
class LivenessData(
    ...
    fun addGenKills(dfcxLiveness: LivenessDataFlow) {
        for (usage in usages) {
            dfcxLiveness.addGen(usage.element, pathToIndex[usage.path]!!)
        }
        for (declaration in declarations) {
            dfcxLiveness.addKill(KillFrom.ScopeEnd, declaration.element, pathToIndex[declaration.path]!!)
        }
    }
