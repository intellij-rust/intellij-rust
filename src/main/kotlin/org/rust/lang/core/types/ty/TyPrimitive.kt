 ...
 *
 * Definition intentionally differs from the reference: we don't treat tuples or arrays as primitive.
 */
...
sealed class TyInteger(override val name: String, val ordinal: Int) : TyNumeric() {
    ...
    // Kotlin `object`s compile into java classes with `INSTANCE` static field and `companion object` fields compile
    // into static field of the host class. Our objects (`U8`, `U16` etc) are extend `TyInteger` class. In java, parent
    // classes are initialized first. So, if we accessing, for example, `U8` object first, we really accessing
    // `U8.INSTANCE` field, that requests to initialize `U8` class, that requests to initialize `TyInteger` before.
    // Then, when we initializing `TyInteger`, `U8` is not initialized and `U8.INSTANCE` is null. So if `VALUES` is a
    // field of `TyInteger` class, it will be filled with null value instead of `U8` We fixing it by moving fields from
    // `companion object` an independent object
    private object TyIntegerValuesHolder {
        val DEFAULT: I32 = TyInteger.I32
        val VALUES: List<TyInteger> = listOf(U8, U16, U32, U64, U128, USize, I8, I16, I32, I64, I128, ISize)
    ...
    companion object {
        ...
        fun fromName(name: String): TyInteger? = VALUES.find { it.name == name }
...
sealed class TyFloat(override val name: String, val ordinal: Int) : TyNumeric() {
    ...
    companion object {
        ...
        fun fromName(name: String): TyFloat? = VALUES.find { it.name == name }
