...
val PsiElement.elementType: IElementType // XXX: be careful not to switch to AST
...
inline fun <reified T : PsiElement> PsiElement.stubChildrenOfType(): List<T> =
    if (this is PsiFileImpl) {
        stub?.childrenStubs?.mapNotNull { it.psi as? T } ?: childrenOfType()
    } else {
