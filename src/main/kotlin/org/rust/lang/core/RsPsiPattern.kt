...
object RsPsiPattern {
    private val STATEMENT_BOUNDARIES: TokenSet =
        TokenSet.create(SEMICOLON, LBRACE, RBRACE)

    val onStatementBeginning: PsiElementPattern.Capture<PsiElement> =
        psiElement().with(OnStatementBeginning())

    val onStruct: PsiElementPattern.Capture<PsiElement> =
        onItem<RsStructItem>()

    val onEnum: PsiElementPattern.Capture<PsiElement> =
        onItem<RsEnumItem>()

    val onFn: PsiElementPattern.Capture<PsiElement> =
        onItem<RsFunction>()

    val onMod: PsiElementPattern.Capture<PsiElement> =
        onItem<RsModItem>() or onItem<RsModDeclItem>()

    val onStatic: PsiElementPattern.Capture<PsiElement> =
        onItem(psiElement<RsConstant>().with("onStaticCondition") { e -> e.kind == RsConstantKind.STATIC })

    val onStaticMut: PsiElementPattern.Capture<PsiElement> =
        onItem(psiElement<RsConstant>().with("onStaticMutCondition") { e -> e.kind == RsConstantKind.MUT_STATIC })

    val onMacro: PsiElementPattern.Capture<PsiElement> =
        onItem<RsMacro>()

    val onTupleStruct: PsiElementPattern.Capture<PsiElement> =
        PlatformPatterns.psiElement()
            .withSuperParent(3, PlatformPatterns.psiElement().withChild(psiElement<RsTupleFields>()))

    val onCrate: PsiElementPattern.Capture<PsiElement> =
        PlatformPatterns.psiElement()
    ...
    val onExternBlock: PsiElementPattern.Capture<PsiElement> =
        onItem<RsForeignModItem>()

    // FIXME: check if this is indeed a foreign function
    val onExternBlockDecl: PsiElementPattern.Capture<PsiElement> =
        onItem<RsFunction>() or onItem<RsConstant>() or onItem<RsForeignModItem>()

    val onAnyItem: PsiElementPattern.Capture<PsiElement> =
        onItem<RsDocAndAttributeOwner>()

    val onExternCrate: PsiElementPattern.Capture<PsiElement> =
        onItem<RsExternCrateItem>()

    val onTrait: PsiElementPattern.Capture<PsiElement> =
        onItem<RsTraitItem>()
    ...
    val onTestFn: PsiElementPattern.Capture<PsiElement> =
        onItem(psiElement<RsFunction>().withChild(psiElement<RsOuterAttr>().withText("#[test]")))
    ...
    val includeMacroLiteral: PsiElementPattern.Capture<RsLitExpr> =
        psiElement<RsLitExpr>().withParent(psiElement<RsIncludeMacroArgument>())

    val whitespace: PsiElementPattern.Capture<PsiElement> =
        psiElement().whitespace()

    val error: PsiElementPattern.Capture<PsiErrorElement> =
...
inline infix fun <reified I : PsiElement> ElementPattern<I>.or(
    pattern: ElementPattern<I>
): PsiElementPattern.Capture<PsiElement> = psiElement().andOr(this, pattern)
...
/**
 * Similar with [TreeElementPattern.afterSiblingSkipping] but it uses [PsiElement.getPrevSibling] to get previous
 * sibling elements instead of [PsiElement.getChildren].
 */
fun <T : PsiElement, Self : PsiElementPattern<T, Self>> PsiElementPattern<T, Self>.withPrevSiblingSkipping(
    ...
    val sibling = e.leftSiblings.dropWhile { skip.accepts(it) }.firstOrNull() ?: return@with false
...
fun <T, Self : ObjectPattern<T, Self>> ObjectPattern<T, Self>.with(
    name: String,
