...
fun <K, V1, V2> zipValues(map1: Map<K, V1>, map2: Map<K, V2>): List<Pair<V1, V2>> =
    map1.mapNotNull { (key, value1) -> map2[key]?.let { value2 -> Pair(value1, value2) } }

inline fun <T> List<T>.singleOrFilter(predicate: (T) -> Boolean): List<T> =
    if (size < 2) this else filter(predicate)

inline fun <T> List<T>.singleOrLet(function: (List<T>) -> List<T>): List<T> =
    if (size < 2) this else function(this)

inline fun <T> List<T>.notEmptyOrLet(function: (List<T>) -> List<T>): List<T> =
    if (isNotEmpty()) this else function(this)
