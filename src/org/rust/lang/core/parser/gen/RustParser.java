...
package org.rust.lang.core.parser.gen;

import com.intellij.lang.PsiBuilder;
import com.intellij.lang.PsiBuilder.Marker;
import static org.rust.lang.core.psi.gen.RustElementTypes.*;
import static org.rust.lang.core.parser.gen.RustParserUtil.*;
...
public class RustParser implements PsiParser, LightPsiParser {
  ...
  /* ********************************************************** */
  // LBRACE (stmt SEMICOLON)* expr? RBRACE
  ...
  static boolean fn_block(PsiBuilder b, int l) {
    if (!recursion_guard_(b, l, "fn_block")) return false;
    if (!nextTokenIs(b, LBRACE)) return false;
    boolean r;
    Marker m = enter_section_(b);
    r = consumeToken(b, LBRACE);
    r = r && fn_block_1(b, l + 1);
    r = r && fn_block_2(b, l + 1);
    r = r && consumeToken(b, RBRACE);
    exit_section_(b, m, null, r);
    return r;
  }
  ...
  // stmt*
  private static boolean fn_block_1(PsiBuilder b, int l) {
    if (!recursion_guard_(b, l, "fn_block_1")) return false;
    int c = current_position_(b);
    while (true) {
      if (!stmt(b, l + 1)) break;
      if (!empty_element_parsed_guard_(b, "fn_block_1", c)) break;
      c = current_position_(b);
    }
    return true;
  }
  ...
  // expr?
  private static boolean fn_block_2(PsiBuilder b, int l) {
    if (!recursion_guard_(b, l, "fn_block_2")) return false;
    expr(b, l + 1, -1);
    return true;
  }

  /* ********************************************************** */
  // FN identifier fn_sig fn_block
