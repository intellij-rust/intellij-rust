// This is a generated file. Not intended for manual editing.
package org.rust.lang.core.psi;

import com.intellij.psi.tree.IElementType;
import com.intellij.psi.PsiElement;
import com.intellij.lang.ASTNode;
import org.rust.lang.core.lexer.RustTokenType;
import org.rust.lang.core.psi.impl.*;

public interface RustCompositeElementTypes {

  IElementType ABI = new RustCompositeElementType("ABI");
  IElementType ANON_PARAM = new RustCompositeElementType("ANON_PARAM");
  IElementType ANON_PARAMS = new RustCompositeElementType("ANON_PARAMS");
  IElementType ARG_LIST = new RustCompositeElementType("ARG_LIST");
  IElementType ARRAY_EXPR = new RustCompositeElementType("ARRAY_EXPR");
  IElementType BINARY_EXPR = new RustCompositeElementType("BINARY_EXPR");
  IElementType BINDING = new RustCompositeElementType("BINDING");
  IElementType BINDINGS = new RustCompositeElementType("BINDINGS");
  IElementType BINDING_MODE = new RustCompositeElementType("BINDING_MODE");
  IElementType BLOCK = new RustCompositeElementType("BLOCK");
  IElementType BLOCK_EXPR = new RustCompositeElementType("BLOCK_EXPR");
  IElementType BOUND = new RustCompositeElementType("BOUND");
  IElementType BOUNDS = new RustCompositeElementType("BOUNDS");
  IElementType BREAK_EXPR = new RustCompositeElementType("BREAK_EXPR");
  IElementType CALL_EXPR = new RustCompositeElementType("CALL_EXPR");
  IElementType CAST_EXPR = new RustCompositeElementType("CAST_EXPR");
  IElementType COMMA_SEPARATED_LIST = new RustCompositeElementType("COMMA_SEPARATED_LIST");
  IElementType CONST_ITEM = new RustCompositeElementType("CONST_ITEM");
  IElementType CONT_EXPR = new RustCompositeElementType("CONT_EXPR");
  IElementType DECL_ITEM = new RustCompositeElementType("DECL_ITEM");
  IElementType DECL_STMT = new RustCompositeElementType("DECL_STMT");
  IElementType ENUM_ARGS = new RustCompositeElementType("ENUM_ARGS");
  IElementType ENUM_DEF = new RustCompositeElementType("ENUM_DEF");
  IElementType ENUM_ITEM = new RustCompositeElementType("ENUM_ITEM");
  IElementType EXPR = new RustCompositeElementType("EXPR");
  IElementType EXPR_STMT = new RustCompositeElementType("EXPR_STMT");
  IElementType EXTERN_BLOCK = new RustCompositeElementType("EXTERN_BLOCK");
  IElementType EXTERN_CRATE_DECL = new RustCompositeElementType("EXTERN_CRATE_DECL");
  IElementType EXTERN_FN_ITEM = new RustCompositeElementType("EXTERN_FN_ITEM");
  IElementType FIELD_EXPR = new RustCompositeElementType("FIELD_EXPR");
  IElementType FN_ITEM = new RustCompositeElementType("FN_ITEM");
  IElementType FN_PARAMS = new RustCompositeElementType("FN_PARAMS");
  IElementType FOREIGN_FN_ITEM = new RustCompositeElementType("FOREIGN_FN_ITEM");
  IElementType FOREIGN_ITEM = new RustCompositeElementType("FOREIGN_ITEM");
  IElementType FOREIGN_MOD_ITEM = new RustCompositeElementType("FOREIGN_MOD_ITEM");
  IElementType FOR_EXPR = new RustCompositeElementType("FOR_EXPR");
  IElementType FOR_LIFETIMES = new RustCompositeElementType("FOR_LIFETIMES");
  IElementType GENERIC_ARGS = new RustCompositeElementType("GENERIC_ARGS");
  IElementType GENERIC_PARAMS = new RustCompositeElementType("GENERIC_PARAMS");
  IElementType GTEQ = new RustCompositeElementType("GTEQ");
  IElementType GTGT = new RustCompositeElementType("GTGT");
  IElementType GTGTEQ = new RustCompositeElementType("GTGTEQ");
  IElementType IF_EXPR = new RustCompositeElementType("IF_EXPR");
  IElementType IF_LET_EXPR = new RustCompositeElementType("IF_LET_EXPR");
  IElementType IMPL_ITEM = new RustCompositeElementType("IMPL_ITEM");
  IElementType IMPL_METHOD = new RustCompositeElementType("IMPL_METHOD");
  IElementType INDEX_EXPR = new RustCompositeElementType("INDEX_EXPR");
  IElementType INNER_ATTR = new RustCompositeElementType("INNER_ATTR");
  IElementType ITEM = new RustCompositeElementType("ITEM");
  IElementType LAMBDA_EXPR = new RustCompositeElementType("LAMBDA_EXPR");
  IElementType LET_DECL = new RustCompositeElementType("LET_DECL");
  IElementType LIFETIMES = new RustCompositeElementType("LIFETIMES");
  IElementType LIT_EXPR = new RustCompositeElementType("LIT_EXPR");
  IElementType LOOP_EXPR = new RustCompositeElementType("LOOP_EXPR");
  IElementType MACRO_ARG = new RustCompositeElementType("MACRO_ARG");
  IElementType MACRO_EXPR = new RustCompositeElementType("MACRO_EXPR");
  IElementType MATCH_EXPR = new RustCompositeElementType("MATCH_EXPR");
  IElementType META_ITEM = new RustCompositeElementType("META_ITEM");
  IElementType METHOD = new RustCompositeElementType("METHOD");
  IElementType METHOD_CALL_EXPR = new RustCompositeElementType("METHOD_CALL_EXPR");
  IElementType MOD_ITEM = new RustCompositeElementType("MOD_ITEM");
  IElementType OUTER_ATTR = new RustCompositeElementType("OUTER_ATTR");
  IElementType PARAM = new RustCompositeElementType("PARAM");
  IElementType PAREN_EXPR = new RustCompositeElementType("PAREN_EXPR");
  IElementType PAT = new RustCompositeElementType("PAT");
  IElementType PATH_EXPR = new RustCompositeElementType("PATH_EXPR");
  IElementType PATH_EXPR_PART = new RustCompositeElementType("PATH_EXPR_PART");
  IElementType PAT_ENUM = new RustCompositeElementType("PAT_ENUM");
  IElementType PAT_IDENT = new RustCompositeElementType("PAT_IDENT");
  IElementType PAT_QUAL_PATH = new RustCompositeElementType("PAT_QUAL_PATH");
  IElementType PAT_RANGE = new RustCompositeElementType("PAT_RANGE");
  IElementType PAT_REG = new RustCompositeElementType("PAT_REG");
  IElementType PAT_STRUCT = new RustCompositeElementType("PAT_STRUCT");
  IElementType PAT_STRUCT_FIELDS = new RustCompositeElementType("PAT_STRUCT_FIELDS");
  IElementType PAT_TUP = new RustCompositeElementType("PAT_TUP");
  IElementType PAT_UNIQ = new RustCompositeElementType("PAT_UNIQ");
  IElementType PAT_VEC = new RustCompositeElementType("PAT_VEC");
  IElementType PAT_WILD = new RustCompositeElementType("PAT_WILD");
  IElementType POLYBOUND = new RustCompositeElementType("POLYBOUND");
  IElementType QUAL_PATH_EXPR = new RustCompositeElementType("QUAL_PATH_EXPR");
  IElementType RANGE_EXPR = new RustCompositeElementType("RANGE_EXPR");
  IElementType RET_EXPR = new RustCompositeElementType("RET_EXPR");
  IElementType RET_TYPE = new RustCompositeElementType("RET_TYPE");
  IElementType SELF_EXPR = new RustCompositeElementType("SELF_EXPR");
  IElementType STATIC_ITEM = new RustCompositeElementType("STATIC_ITEM");
  IElementType STMT = new RustCompositeElementType("STMT");
  IElementType STMT_ITEM = new RustCompositeElementType("STMT_ITEM");
  IElementType STRUCT_DECL_ARGS = new RustCompositeElementType("STRUCT_DECL_ARGS");
  IElementType STRUCT_DECL_FIELD = new RustCompositeElementType("STRUCT_DECL_FIELD");
  IElementType STRUCT_EXPR = new RustCompositeElementType("STRUCT_EXPR");
  IElementType STRUCT_EXPR_BODY = new RustCompositeElementType("STRUCT_EXPR_BODY");
  IElementType STRUCT_ITEM = new RustCompositeElementType("STRUCT_ITEM");
  IElementType STRUCT_TUPLE_ARGS = new RustCompositeElementType("STRUCT_TUPLE_ARGS");
  IElementType STRUCT_TUPLE_FIELD = new RustCompositeElementType("STRUCT_TUPLE_FIELD");
  IElementType TRAIT_CONST = new RustCompositeElementType("TRAIT_CONST");
  IElementType TRAIT_ITEM = new RustCompositeElementType("TRAIT_ITEM");
  IElementType TRAIT_METHOD = new RustCompositeElementType("TRAIT_METHOD");
  IElementType TRAIT_REF = new RustCompositeElementType("TRAIT_REF");
  IElementType TRAIT_TYPE = new RustCompositeElementType("TRAIT_TYPE");
  IElementType TUPLE_EXPR = new RustCompositeElementType("TUPLE_EXPR");
  IElementType TYPE_ASCRIPTION = new RustCompositeElementType("TYPE_ASCRIPTION");
  IElementType TYPE_ITEM = new RustCompositeElementType("TYPE_ITEM");
  IElementType TYPE_METHOD = new RustCompositeElementType("TYPE_METHOD");
  IElementType TYPE_PARAM = new RustCompositeElementType("TYPE_PARAM");
  IElementType TYPE_PARAM_BOUNDS = new RustCompositeElementType("TYPE_PARAM_BOUNDS");
  IElementType TYPE_PARAM_DEFAULT = new RustCompositeElementType("TYPE_PARAM_DEFAULT");
  IElementType TYPE_PRIM_SUM = new RustCompositeElementType("TYPE_PRIM_SUM");
  IElementType TYPE_SUM = new RustCompositeElementType("TYPE_SUM");
  IElementType TYPE_SUMS = new RustCompositeElementType("TYPE_SUMS");
  IElementType UNARY_EXPR = new RustCompositeElementType("UNARY_EXPR");
  IElementType UNIT_EXPR = new RustCompositeElementType("UNIT_EXPR");
  IElementType USE_ITEM = new RustCompositeElementType("USE_ITEM");
  IElementType VIEW_PATH = new RustCompositeElementType("VIEW_PATH");
  IElementType VIEW_PATH_PART = new RustCompositeElementType("VIEW_PATH_PART");
  IElementType VIEW_PATH_PART_LEFTISH = new RustCompositeElementType("VIEW_PATH_PART_LEFTISH");
  IElementType WHERE_CLAUSE = new RustCompositeElementType("WHERE_CLAUSE");
  IElementType WHERE_PRED = new RustCompositeElementType("WHERE_PRED");
  IElementType WHILE_EXPR = new RustCompositeElementType("WHILE_EXPR");
  IElementType WHILE_LET_EXPR = new RustCompositeElementType("WHILE_LET_EXPR");

  class Factory {
    public static PsiElement createElement(ASTNode node) {
      IElementType type = node.getElementType();
       if (type == ABI) {
        return new RustAbiImpl(node);
      }
      else if (type == ANON_PARAM) {
        return new RustAnonParamImpl(node);
      }
      else if (type == ANON_PARAMS) {
        return new RustAnonParamsImpl(node);
      }
      else if (type == ARG_LIST) {
        return new RustArgListImpl(node);
      }
      else if (type == ARRAY_EXPR) {
        return new RustArrayExprImpl(node);
      }
      else if (type == BINARY_EXPR) {
        return new RustBinaryExprImpl(node);
      }
      else if (type == BINDING) {
        return new RustBindingImpl(node);
      }
      else if (type == BINDINGS) {
        return new RustBindingsImpl(node);
      }
      else if (type == BINDING_MODE) {
        return new RustBindingModeImpl(node);
      }
      else if (type == BLOCK) {
        return new RustBlockImpl(node);
      }
      else if (type == BLOCK_EXPR) {
        return new RustBlockExprImpl(node);
      }
      else if (type == BOUND) {
        return new RustBoundImpl(node);
      }
      else if (type == BOUNDS) {
        return new RustBoundsImpl(node);
      }
      else if (type == BREAK_EXPR) {
        return new RustBreakExprImpl(node);
      }
      else if (type == CALL_EXPR) {
        return new RustCallExprImpl(node);
      }
      else if (type == CAST_EXPR) {
        return new RustCastExprImpl(node);
      }
      else if (type == COMMA_SEPARATED_LIST) {
        return new RustCommaSeparatedListImpl(node);
      }
      else if (type == CONST_ITEM) {
        return new RustConstItemImpl(node);
      }
      else if (type == CONT_EXPR) {
        return new RustContExprImpl(node);
      }
      else if (type == DECL_ITEM) {
        return new RustDeclItemImpl(node);
      }
      else if (type == DECL_STMT) {
        return new RustDeclStmtImpl(node);
      }
      else if (type == ENUM_ARGS) {
        return new RustEnumArgsImpl(node);
      }
      else if (type == ENUM_DEF) {
        return new RustEnumDefImpl(node);
      }
      else if (type == ENUM_ITEM) {
        return new RustEnumItemImpl(node);
      }
      else if (type == EXPR) {
        return new RustExprImpl(node);
      }
      else if (type == EXPR_STMT) {
        return new RustExprStmtImpl(node);
      }
      else if (type == EXTERN_BLOCK) {
        return new RustExternBlockImpl(node);
      }
      else if (type == EXTERN_CRATE_DECL) {
        return new RustExternCrateDeclImpl(node);
      }
      else if (type == EXTERN_FN_ITEM) {
        return new RustExternFnItemImpl(node);
      }
      else if (type == FIELD_EXPR) {
        return new RustFieldExprImpl(node);
      }
      else if (type == FN_ITEM) {
        return new RustFnItemImpl(node);
      }
      else if (type == FN_PARAMS) {
        return new RustFnParamsImpl(node);
      }
      else if (type == FOREIGN_FN_ITEM) {
        return new RustForeignFnItemImpl(node);
      }
      else if (type == FOREIGN_ITEM) {
        return new RustForeignItemImpl(node);
      }
      else if (type == FOREIGN_MOD_ITEM) {
        return new RustForeignModItemImpl(node);
      }
      else if (type == FOR_EXPR) {
        return new RustForExprImpl(node);
      }
      else if (type == FOR_LIFETIMES) {
        return new RustForLifetimesImpl(node);
      }
      else if (type == GENERIC_ARGS) {
        return new RustGenericArgsImpl(node);
      }
      else if (type == GENERIC_PARAMS) {
        return new RustGenericParamsImpl(node);
      }
      else if (type == GTEQ) {
        return new RustGteqImpl(node);
      }
      else if (type == GTGT) {
        return new RustGtgtImpl(node);
      }
      else if (type == GTGTEQ) {
        return new RustGtgteqImpl(node);
      }
      else if (type == IF_EXPR) {
        return new RustIfExprImpl(node);
      }
      else if (type == IF_LET_EXPR) {
        return new RustIfLetExprImpl(node);
      }
      else if (type == IMPL_ITEM) {
        return new RustImplItemImpl(node);
      }
      else if (type == IMPL_METHOD) {
        return new RustImplMethodImpl(node);
      }
      else if (type == INDEX_EXPR) {
        return new RustIndexExprImpl(node);
      }
      else if (type == INNER_ATTR) {
        return new RustInnerAttrImpl(node);
      }
      else if (type == ITEM) {
        return new RustItemImpl(node);
      }
      else if (type == LAMBDA_EXPR) {
        return new RustLambdaExprImpl(node);
      }
      else if (type == LET_DECL) {
        return new RustLetDeclImpl(node);
      }
      else if (type == LIFETIMES) {
        return new RustLifetimesImpl(node);
      }
      else if (type == LIT_EXPR) {
        return new RustLitExprImpl(node);
      }
      else if (type == LOOP_EXPR) {
        return new RustLoopExprImpl(node);
      }
      else if (type == MACRO_ARG) {
        return new RustMacroArgImpl(node);
      }
      else if (type == MACRO_EXPR) {
        return new RustMacroExprImpl(node);
      }
      else if (type == MATCH_EXPR) {
        return new RustMatchExprImpl(node);
      }
      else if (type == META_ITEM) {
        return new RustMetaItemImpl(node);
      }
      else if (type == METHOD) {
        return new RustMethodImpl(node);
      }
      else if (type == METHOD_CALL_EXPR) {
        return new RustMethodCallExprImpl(node);
      }
      else if (type == MOD_ITEM) {
        return new RustModItemImpl(node);
      }
      else if (type == OUTER_ATTR) {
        return new RustOuterAttrImpl(node);
      }
      else if (type == PARAM) {
        return new RustParamImpl(node);
      }
      else if (type == PAREN_EXPR) {
        return new RustParenExprImpl(node);
      }
      else if (type == PAT) {
        return new RustPatImpl(node);
      }
      else if (type == PATH_EXPR) {
        return new RustPathExprImpl(node);
      }
      else if (type == PATH_EXPR_PART) {
        return new RustPathExprPartImpl(node);
      }
      else if (type == PAT_ENUM) {
        return new RustPatEnumImpl(node);
      }
      else if (type == PAT_IDENT) {
        return new RustPatIdentImpl(node);
      }
      else if (type == PAT_QUAL_PATH) {
        return new RustPatQualPathImpl(node);
      }
      else if (type == PAT_RANGE) {
        return new RustPatRangeImpl(node);
      }
      else if (type == PAT_REG) {
        return new RustPatRegImpl(node);
      }
      else if (type == PAT_STRUCT) {
        return new RustPatStructImpl(node);
      }
      else if (type == PAT_STRUCT_FIELDS) {
        return new RustPatStructFieldsImpl(node);
      }
      else if (type == PAT_TUP) {
        return new RustPatTupImpl(node);
      }
      else if (type == PAT_UNIQ) {
        return new RustPatUniqImpl(node);
      }
      else if (type == PAT_VEC) {
        return new RustPatVecImpl(node);
      }
      else if (type == PAT_WILD) {
        return new RustPatWildImpl(node);
      }
      else if (type == POLYBOUND) {
        return new RustPolyboundImpl(node);
      }
      else if (type == QUAL_PATH_EXPR) {
        return new RustQualPathExprImpl(node);
      }
      else if (type == RANGE_EXPR) {
        return new RustRangeExprImpl(node);
      }
      else if (type == RET_EXPR) {
        return new RustRetExprImpl(node);
      }
      else if (type == RET_TYPE) {
        return new RustRetTypeImpl(node);
      }
      else if (type == SELF_EXPR) {
        return new RustSelfExprImpl(node);
      }
      else if (type == STATIC_ITEM) {
        return new RustStaticItemImpl(node);
      }
      else if (type == STMT) {
        return new RustStmtImpl(node);
      }
      else if (type == STMT_ITEM) {
        return new RustStmtItemImpl(node);
      }
      else if (type == STRUCT_DECL_ARGS) {
        return new RustStructDeclArgsImpl(node);
      }
      else if (type == STRUCT_DECL_FIELD) {
        return new RustStructDeclFieldImpl(node);
      }
      else if (type == STRUCT_EXPR) {
        return new RustStructExprImpl(node);
      }
      else if (type == STRUCT_EXPR_BODY) {
        return new RustStructExprBodyImpl(node);
      }
      else if (type == STRUCT_ITEM) {
        return new RustStructItemImpl(node);
      }
      else if (type == STRUCT_TUPLE_ARGS) {
        return new RustStructTupleArgsImpl(node);
      }
      else if (type == STRUCT_TUPLE_FIELD) {
        return new RustStructTupleFieldImpl(node);
      }
      else if (type == TRAIT_CONST) {
        return new RustTraitConstImpl(node);
      }
      else if (type == TRAIT_ITEM) {
        return new RustTraitItemImpl(node);
      }
      else if (type == TRAIT_METHOD) {
        return new RustTraitMethodImpl(node);
      }
      else if (type == TRAIT_REF) {
        return new RustTraitRefImpl(node);
      }
      else if (type == TRAIT_TYPE) {
        return new RustTraitTypeImpl(node);
      }
      else if (type == TUPLE_EXPR) {
        return new RustTupleExprImpl(node);
      }
      else if (type == TYPE_ASCRIPTION) {
        return new RustTypeAscriptionImpl(node);
      }
      else if (type == TYPE_ITEM) {
        return new RustTypeItemImpl(node);
      }
      else if (type == TYPE_METHOD) {
        return new RustTypeMethodImpl(node);
      }
      else if (type == TYPE_PARAM) {
        return new RustTypeParamImpl(node);
      }
      else if (type == TYPE_PARAM_BOUNDS) {
        return new RustTypeParamBoundsImpl(node);
      }
      else if (type == TYPE_PARAM_DEFAULT) {
        return new RustTypeParamDefaultImpl(node);
      }
      else if (type == TYPE_PRIM_SUM) {
        return new RustTypePrimSumImpl(node);
      }
      else if (type == TYPE_SUM) {
        return new RustTypeSumImpl(node);
      }
      else if (type == TYPE_SUMS) {
        return new RustTypeSumsImpl(node);
      }
      else if (type == UNARY_EXPR) {
        return new RustUnaryExprImpl(node);
      }
      else if (type == UNIT_EXPR) {
        return new RustUnitExprImpl(node);
      }
      else if (type == USE_ITEM) {
        return new RustUseItemImpl(node);
      }
      else if (type == VIEW_PATH) {
        return new RustViewPathImpl(node);
      }
      else if (type == VIEW_PATH_PART) {
        return new RustViewPathPartImpl(node);
      }
      else if (type == VIEW_PATH_PART_LEFTISH) {
        return new RustViewPathPartLeftishImpl(node);
      }
      else if (type == WHERE_CLAUSE) {
        return new RustWhereClauseImpl(node);
      }
      else if (type == WHERE_PRED) {
        return new RustWherePredImpl(node);
      }
      else if (type == WHILE_EXPR) {
        return new RustWhileExprImpl(node);
      }
      else if (type == WHILE_LET_EXPR) {
        return new RustWhileLetExprImpl(node);
      }
      throw new AssertionError("Unknown element type: " + type);
    }
  }
}
